...
export default function useImportThumbnails() {
    const [thumbnails, setThumbnails] = useState<typeof cachedData>(cachedData);
            ...
            fetch("https://art-gallery-backend-fv6z.onrender.com/api/images")
                ...
                .then((json: { name: string; id: string }[]) => {
                    const mapped = json.map(img => ({
                        ...img,
                        url: `https://art-gallery-backend-fv6z.onrender.com/api/thumbnail/${img.id}`
                    }));
                    cachedData = mapped;
